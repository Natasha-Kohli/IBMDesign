...
def get_adj_clusters(lat, lon, radius, cursor):
    ...
    print(cursor.mogrify(query, {'lat' : lat, 'lon' : lon, 'radius' : radius}))
...
def get_row_with_pred(row, clf, day, month, chunkIdx):
    return (row[0], row[1], clf.predict([[row[0], row[1], day, month, chunkIdx]]).tolist())

def get_ranked_clusters(adj_clusters, day, month, chunkIdx, limit):
    clf = get_clf()
    clusters = [get_row_with_pred(row, clf, day, month, chunkIdx) for row in adj_clusters]
    clusters.sort(reverse=True, key=(lambda row : row[2]))
...
def recommend(lat, lon, radius, nrows, day, month, hour, minute):    
    try:
        ...
        return get_ranked_clusters(adj_clusters, day, month, chunkIdx, nrows)
